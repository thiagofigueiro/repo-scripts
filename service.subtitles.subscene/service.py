...
LANGUAGE = ADD_ON.getLocalizedString
...
def getallsubs(url, allowed_languages, filename="", episode=""):
    ...
    if len(codes) < 1:
        xbmc.executebuiltin((u'Notification(%s,%s)' % (SCRIPT_NAME, LANGUAGE(32004))).encode('utf-8'))
        return
        ...
        if language_info and language_info['3let'] in allowed_languages:
            ...
            if episode != "":
                ...
                if episode_regex.search(subtitle_name):
                    ...
                    subtitle_name = subtitle_name + ' ' + (LANGUAGE(32001) % int(matches.group('numfiles')))
                    subtitles.append({'rating': rating, 'filename': subtitle_name, 'sync': sync, 'link': link,
                                      'lang': language_info, 'hearing_imp': hearing_imp, 'comment': comment,
                                      'episode': episode})
            else:
...
def download(link, episode=""):
    ...
    if match:
        ...
        if len(subtitle_list) == 0:
            if episode:
                xbmc.executebuiltin((u'Notification(%s,%s)' % (SCRIPT_NAME, LANGUAGE(32002))).encode('utf-8'))
            else:
                xbmc.executebuiltin((u'Notification(%s,%s)' % (SCRIPT_NAME, LANGUAGE(32003))).encode('utf-8'))
